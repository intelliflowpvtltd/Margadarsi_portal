...
<?php $__env->startSection('title', 'Lead Statuses - Master Data'); ?>

<?php $__env->startSection('content'); ?>
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 fw-bold">Lead Statuses</h4>
        ...
        <button class="btn btn-primary" id="btnAdd"><i class="bi bi-plus-lg me-2"></i>Add Lead Status</button>
    </div>
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-hover" id="dataTable">
                    <thead><tr><th>#</th><th>Name</th><th>Color</th><th>Order</th><th>Type</th><th>Status</th><th>Actions</th></tr></thead>
            ...
            <div id="emptyState" class="text-center py-5 d-none"><i class="bi bi-flag display-4 text-muted"></i><p class="mt-2 text-muted">No lead statuses found</p></div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="itemModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title" id="modalTitle">Add Lead Status</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <form id="itemForm">
                    <input type="hidden" id="itemId">
                    <div class="mb-3"><label class="form-label">Name <span class="text-danger">*</span></label><input type="text" class="form-control" name="name" id="itemName" placeholder="e.g., New, Contacted, Qualified" required></div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3"><div class="form-check"><input class="form-check-input" type="checkbox" id="isPipeline" name="is_pipeline"><label class="form-check-label" for="isPipeline">Pipeline Stage</label></div></div>
                        <div class="col-md-6 mb-3"><div class="form-check"><input class="form-check-input" type="checkbox" id="isFinal" name="is_final"><label class="form-check-label" for="isFinal">Final Status</label></div></div>
                    </div>
...
<script>
const API_BASE='/admin/masters/lead-statuses';let itemsData=[];
...
function renderItems(){const tbody=document.getElementById('dataBody');if(!itemsData||!itemsData.length){showEmpty();return}tbody.innerHTML=itemsData.map((i,x)=>`<tr><td>${x+1}</td><td><strong>${i.name}</strong></td><td><span class="color-dot" style="background:${i.color||'#6c757d'}"></span></td><td>${i.display_order||0}</td><td>${i.is_pipeline?'<span class="badge bg-info">Pipeline</span>':''} ${i.is_final?'<span class="badge bg-warning">Final</span>':''}</td><td><span class="badge ${i.is_active?'badge-active':'badge-inactive'}">${i.is_active?'Active':'Inactive'}</span></td><td><button class="btn btn-sm btn-outline-primary btn-action me-1" onclick="editItem(${i.id})"><i class="bi bi-pencil"></i></button><button class="btn btn-sm btn-outline-danger btn-action" onclick="deleteItem(${i.id})"><i class="bi bi-trash"></i></button></td></tr>`).join('');hideLoading();document.getElementById('dataTable').classList.remove('d-none')}
function openCreateModal(){document.getElementById('modalTitle').textContent='Add Lead Status';document.getElementById('itemForm').reset();document.getElementById('itemId').value='';document.getElementById('itemColor').value='#6c757d';document.getElementById('itemActive').checked=true;new bootstrap.Modal(document.getElementById('itemModal')).show()}
function editItem(id){const i=itemsData.find(x=>x.id===id);if(!i)return;document.getElementById('modalTitle').textContent='Edit Lead Status';document.getElementById('itemId').value=i.id;document.getElementById('itemName').value=i.name;document.getElementById('itemColor').value=i.color||'#6c757d';document.getElementById('itemOrder').value=i.display_order||0;document.getElementById('isPipeline').checked=i.is_pipeline;document.getElementById('isFinal').checked=i.is_final;document.getElementById('itemActive').checked=i.is_active;new bootstrap.Modal(document.getElementById('itemModal')).show()}
async function saveItem(){const id=document.getElementById('itemId').value;const data={name:document.getElementById('itemName').value,color:document.getElementById('itemColor').value,display_order:parseInt(document.getElementById('itemOrder').value)||0,is_pipeline:document.getElementById('isPipeline').checked,is_final:document.getElementById('isFinal').checked,is_active:document.getElementById('itemActive').checked};try{const r=await fetch(id?`${API_BASE}/${id}`:API_BASE,{method:id?'PUT':'POST',headers:{'Accept':'application/json','Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content,'X-Requested-With':'XMLHttpRequest'},body:JSON.stringify(data),credentials:'same-origin'});const res=await r.json();if(!r.ok)throw new Error(res.message);bootstrap.Modal.getInstance(document.getElementById('itemModal')).hide();loadItems();alert(res.message||'Saved!')}catch(e){alert(e.message||'Error')}}
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Margadarsi_portal\resources\views/masters/lead-statuses.blade.php ENDPATH**/ ?>