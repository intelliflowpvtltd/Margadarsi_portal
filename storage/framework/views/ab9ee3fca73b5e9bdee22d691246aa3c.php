...
<?php $__env->startSection('title', 'Property Statuses - Master Data'); ?>

<?php $__env->startSection('content'); ?>
<div class="container-fluid py-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 fw-bold">Property Statuses</h4>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="<?php echo e(route('dashboard')); ?>">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="<?php echo e(route('masters.index')); ?>">Masters</a></li>
                    <li class="breadcrumb-item active">Property Statuses</li>
                </ol>
            </nav>
        </div>
        <button class="btn btn-primary" id="btnAdd"><i class="bi bi-plus-lg me-2"></i>Add Status</button>
    </div>
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-hover" id="dataTable">
                    <thead><tr><th>#</th><th>Name</th><th>Color</th><th>Property Type</th><th>Status</th><th>Actions</th></tr></thead>
            ...
            <div id="emptyState" class="text-center py-5 d-none"><i class="bi bi-flag display-4 text-muted"></i><p class="mt-2 text-muted">No property statuses found</p></div>
        </div>
    </div>
</div>
...
<div class="modal fade" id="itemModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header"><h5 class="modal-title" id="modalTitle">Add Status</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div>
            <div class="modal-body">
                <form id="itemForm">
                    <input type="hidden" id="itemId">
                    <div class="mb-3"><label class="form-label">Name <span class="text-danger">*</span></label><input type="text" class="form-control" name="name" id="itemName" placeholder="e.g., Available, Sold, Reserved" required></div>
                    <div class="mb-3"><label class="form-label">Property Type</label><select class="form-select" name="property_type_id" id="propertyTypeSelect"><option value="">All Types</option></select></div>
...
<script>
const API_BASE='/admin/masters/property-statuses',TYPES_API='/admin/masters/property-types';let itemsData=[],typesData=[];
document.addEventListener('DOMContentLoaded',()=>{loadTypes();loadItems();document.getElementById('btnAdd').addEventListener('click',openCreateModal);document.getElementById('btnSave').addEventListener('click',saveItem)});
async function loadTypes(){try{const r=await fetch(TYPES_API,{headers:{'Accept':'application/json','X-Requested-With':'XMLHttpRequest'},credentials:'same-origin'});const d=await r.json();typesData=d.data||d;document.getElementById('propertyTypeSelect').innerHTML='<option value="">All Types</option>'+typesData.map(t=>`<option value="${t.id}">${t.name}</option>`).join('')}catch(e){}}
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\Margadarsi_portal\resources\views/masters/property-statuses.blade.php ENDPATH**/ ?>