...
@section('title', 'Lead Statuses - Master Data')
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="itemForm">
                    ...
                    <div class="mb-3"><label class="form-label">Color</label><input type="color" class="form-control form-control-color" name="color_code" id="itemColor" value="#6c757d"></div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3"><div class="form-check"><input class="form-check-input" type="checkbox" id="isPipeline" name="is_pipeline_state"><label class="form-check-label" for="isPipeline">Pipeline Stage</label></div></div>
                        <div class="col-md-6 mb-3"><div class="form-check"><input class="form-check-input" type="checkbox" id="isFinal" name="is_final_state"><label class="form-check-label" for="isFinal">Final Status</label></div></div>
                    </div>
...
function editItem(id){const i=itemsData.find(x=>x.id===id);if(!i)return;document.getElementById('modalTitle').textContent='Edit Lead Status';document.getElementById('itemId').value=i.id;document.getElementById('itemName').value=i.name;document.getElementById('itemColor').value=i.color_code||'#6c757d';document.getElementById('itemOrder').value=i.sort_order||0;document.getElementById('isPipeline').checked=i.is_pipeline_state;document.getElementById('isFinal').checked=i.is_final_state;document.getElementById('itemActive').checked=i.is_active;new bootstrap.Modal(document.getElementById('itemModal')).show()}
async function saveItem(){const id=document.getElementById('itemId').value;const data={name:document.getElementById('itemName').value,color_code:document.getElementById('itemColor').value,sort_order:parseInt(document.getElementById('itemOrder').value)||0,is_pipeline_state:document.getElementById('isPipeline').checked,is_final_state:document.getElementById('isFinal').checked,is_active:document.getElementById('itemActive').checked};try{const r=await fetch(id?`${API_BASE}/${id}`:API_BASE,{method:id?'PUT':'POST',headers:{'Accept':'application/json','Content-Type':'application/json','X-CSRF-TOKEN':document.querySelector('meta[name="csrf-token"]').content,'X-Requested-With':'XMLHttpRequest'},body:JSON.stringify(data),credentials:'same-origin'});const res=await r.json();if(!r.ok)throw new Error(res.message);bootstrap.Modal.getInstance(document.getElementById('itemModal')).hide();loadItems();alert(res.message||'Saved!')}catch(e){alert(e.message||'Error')}}
